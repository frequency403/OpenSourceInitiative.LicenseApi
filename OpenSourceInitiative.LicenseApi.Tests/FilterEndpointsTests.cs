...
public class FilterEndpointsTests
{
    ...
    public static IEnumerable<ITheoryDataRow> SuccessCases()
    {
        // name=mit
        yield return new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string, string, Dictionary<string, string>, string[], Dictionary<string, string>>
        (
            // call
            c => c.GetLicensesByNameAsync("mit"),
            // first request url
        ...
        yield return new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string, string, Dictionary<string, string>, string[], Dictionary<string, string>>
        (
            c => c.GetLicensesByKeywordAsync("popular-strong-community"),
        ...
        yield return new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string, string, Dictionary<string, string>, string[], Dictionary<string, string>>
        (
            c => c.GetLicensesByKeywordAsync(OsiLicenseKeyword.PopularStrongCommunity),
            "https://opensource.org/api/license?keyword=popular-strong-community",
            "[{\"id\":\"mit\",\"name\":\"MIT License\",\"spdx_id\":\"MIT\",\"_links\":{\"self\":{\"href\":\"s\"},\"html\":{\"href\":\"https://opensource.org/license/mit/\"},\"collection\":{\"href\":\"c\"}}}]",
        ...
        yield return new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string, string, Dictionary<string, string>, string[], Dictionary<string, string>>
        (
            c => c.GetLicensesByStewardAsync("eclipse-foundation"),
        ...
        yield return new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string, string, Dictionary<string, string>, string[], Dictionary<string, string>>
        (
            c => c.GetLicensesBySpdxPatternAsync("gpl*"),
    ...
    public static IEnumerable<ITheoryDataRow> EdgeCases()
    {
        // Empty or whitespace => empty result (guard path) for string overloads
        yield return new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string?, string?, Dictionary<string, string>, int>
                     (
            c => c.GetLicensesByNameAsync(""),
            null!,
            null!,
            new Dictionary<string, string>(),
            0
        ...
        yield return
            new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string?, string?, Dictionary<string, string>, int>
            (
                c => c.GetLicensesByStewardAsync(" \t"),
                null!,
                null!,
                new Dictionary<string, string>(),
                0
        ...
        yield return
            new TheoryDataRow<Func<OsiLicensesClient, Task<IReadOnlyList<OsiLicense>>>, string?, string?, Dictionary<string, string>, int>
            (
                c => c.GetLicensesBySpdxPatternAsync(""),
                null!,
                null!,
                new Dictionary<string, string>(),
                0
        ...
        Dictionary<string, string> expectedTexts)
    {
        ...
        list.Select(l => l.SpdxId).ShouldBe(expectedSpdxIds, ignoreOrder: true);
        foreach (var license in list)
        {
            if (license.SpdxId != null && expectedTexts.TryGetValue(license.SpdxId, out var expectedText))
            {
                license.LicenseText.ShouldBe(expectedText);
            }
        }
    }
        ...
        int expectedCount)
    {
        ...
        {
            await using var client = new OsiLicensesClient(osiClient);
            var ex = await Should.ThrowAsync<Exception>(() => call(client));
            (ex is HttpRequestException or System.Text.Json.JsonException).ShouldBeTrue();
        }
        else
        {
            await using var client = new OsiLicensesClient(osiClient);
